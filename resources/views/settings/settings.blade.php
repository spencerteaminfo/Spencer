                                ...
                                <div class="card border-0 shadow-sm rounded-4 p-4">
                                    ...
                                    <form id="settingsForm">
                                        ...
                                        @foreach ($allSettings as $setting)
                                            @php
                                                $settingLabels = [
                                                    'email_notifications' => __('setting.email_notifications'),
                                                    'hide_pfp' => __('setting.hide_pfp'),
                                                    'language' => __('setting.language'),
                                                    'theme' => __('setting.theme'),
                                                ];
                                                $optionLabels = [
                                                    'light' => __('setting.theme_light'),
                                                    'dark' => __('setting.theme_dark'),
                                                    'enable' => __('setting.enable'),
                                                    'disable' => __('setting.disable'),
                                                    'show' => __('setting.show'),
                                                    'hide' => __('setting.hide'),
                                                    'czech' => __('setting.czech'),
                                                    'english' => __('setting.english'),
                                                    'german' => __('setting.german'),
                                                ];
                                                $settingName = $settingLabels[$setting->name] ?? ucwords(str_replace('_', ' ', $setting->name));
                                            @endphp
                                            <div class="d-flex justify-content-between align-items-center mb-4">
                                            <span class="fw-medium text-dark">{{ $settingName }}</span>
                                                    ...
                                                    <div class="dropdown">
                                                        <select name="options[{{ $setting->id }}]" class="form-select form-select-sm setting-input border-primary text-primary fw-bold">
                                                            @foreach ($setting->options as $option)
                                                                @php
                                                                    $dName = $optionLabels[$option->option_data] ?? ucwords(str_replace('_', ' ', $option->option_data));
                                                                @endphp
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mb-4">
                                        <span class="fw-medium text-dark">{{__('setting.hide_pfp')}}</span>
                                        <a href="#" id="DeletePFP" class="link-underline link-underline-opacity-0 link-danger link-underline-opacity-0-hover">{{__('setting.hide_pfp')}} <img src="{{ Vite::asset('resources/svg/trash.svg') }}" class="mb-1" alt="trashIcon" width="16" height="16"></a>
                                    </div>
